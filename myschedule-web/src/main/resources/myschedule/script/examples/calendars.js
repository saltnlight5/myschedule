...
job = scheduler.createJobDetail('CalJob', Packages.myschedule.quartz.extra.job.LoggerJob);
trigger = scheduler.createCronTrigger('CalJob1', '0 0 * * * ?');
trigger.setCalendarName('SkipWeekEnd');
scheduler.scheduleJob(job, trigger);

job = scheduler.createJobDetail('CalJob2', Packages.myschedule.quartz.extra.job.LoggerJob);
trigger = scheduler.createCronTrigger('CalJob2', '0 0 * * * ?');
trigger.setCalendarName('SkipJan');
scheduler.scheduleJob(job, trigger);

job = scheduler.createJobDetail('CalJob3',Packages. myschedule.quartz.extra.job.LoggerJob);
