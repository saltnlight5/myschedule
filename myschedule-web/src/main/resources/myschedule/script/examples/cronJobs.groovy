...
scheduler.scheduleCronJob("hourlyCronJob", "0 0 * ? * MON-FRI", myschedule.quartz.extra.job.LoggerJob.class)

// Schedule minutely job on JUN and DEC only
scheduler.scheduleCronJob("minutelyCronJob", "0 * * * JUN,DEC ?", myschedule.quartz.extra.job.LoggerJob.class)

// Schedule secondly job
scheduler.scheduleCronJob("secondlyCronJob", "* * * * * ?", myschedule.quartz.extra.job.LoggerJob.class)

// Schedule hourly job with job data and start time of 20s delay.
scheduler.scheduleCronJob("hourlyCronJobWithStartTimeDelay", "0 0 * * * ?", myschedule.quartz.extra.job.LoggerJob.class, 
		scheduler.mkMap('color', 'RED'), 
		new Date(System.currentTimeMillis() + 20 * 1000))

// Schedule one job with multiple triggers
import org.quartz.*
job = scheduler.createJobDetail(JobKey.jobKey("jobWithMutltipleTriggers2"), myschedule.quartz.extra.job.LoggerJob.class, true, null)
...
job = JobBuilder
	.newJob(myschedule.quartz.extra.job.LoggerJob.class)
